...
import 'package:smit/ui/dava/update_dava.dart';
...
class _BillDavaListState extends State<BillDavaList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: false,
        title: CommonText.extraBold(
          "${widget.name}-${widget.year}".toString(),
          size: 18,
        ),
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection("Dava").where('month', isEqualTo: widget.month).where('year', isEqualTo: widget.year).orderBy('time', descending: true).snapshots(),
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return InkWell(
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => UpdateDava(data: data),));
                        },
                        child: Card(
                          clipBehavior: Clip.hardEdge,
                          margin: const EdgeInsets.symmetric(horizontal: 7, vertical: 5),
        ...
        onPressed: () {
          if (widget.ind == 0) {
            Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const AddDavaKhatar(),
                ));
          }
        },
        tooltip: "Add bill",
