...
class UpdateKhatar extends StatefulWidget {
  var data;
  UpdateKhatar({super.key,required this.data});

  @override
  State<UpdateKhatar> createState() => _UpdateKhatarState();
}

class _UpdateKhatarState extends State<UpdateKhatar> {
  ...
  void updateBillToFirestore(
    ...
    try {
      await firestore.collection("Khatar").doc(widget.data.id.toString()).update({
        // 'id':widget.catId,
        'date': dateTime,
        'month':picked!.month,
        'year':picked!.year,
  ...
  void deleteBillToFirestore() async {
    FirebaseFirestore firestore = FirebaseFirestore.instance;
    try {
      await firestore.collection("Khatar").doc(widget.data.id.toString()).delete();
